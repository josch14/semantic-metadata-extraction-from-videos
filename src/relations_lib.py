...
from .entities_lib import pronoun_resolution, extract_nouns_from_doc
...
def is_preposition(token: spacy.tokens.Token):
    return token.text.lower() in PREPOSITIONS


def is_entity_or_pronoun(token: spacy.tokens.Token, tokens_for_entities: list):
...
def extract_relations(doc: spacy.tokens.Doc,
    ...
    """
    main method for relation extraction: extract relations from a spacy doc and corresponding list of timestamps
    """
    # determine entities
    ...
    for token in doc:
        if token.pos_ in VERB_TAGS:
            ...
            # (b) verb is something like "'s" and "'re", which is found by spaCy as verb and lemmatized to "be". Such verbs
            #     are not recognized by WordNet as verbs. In this case, we ignore WordNet
    ...
    tuples = []
    for verb in verbs:
        subject = find_subject_for_verb(verb, tokens_for_entities)
    ...
    for subject, verb in tuples:
        modifiers_of_objects, objects = find_objects_for_verb(verb, tokens_for_entities, wn_dictionary)
    ...
    for subject, verb, modifiers, object in candidate_relations:
        relations.append([
            find_conjunct_tokens_of_entities_or_prons(subject, tokens_for_entities),
            verb,
            modifiers,
            find_conjunct_tokens_of_entities_or_prons(object, tokens_for_entities)
        ])
    ...
    for subjects, verb, modifiers, objects in relations:
        ...
        sentence_index = get_sentence_idx_for_token(doc, verb)
        assert all([sentence_index == get_sentence_idx_for_token(doc, t) for t in subjects]) \
               and all([sentence_index == get_sentence_idx_for_token(doc, t) for t in modifiers]) \
               and all([sentence_index == get_sentence_idx_for_token(doc, t) for t in objects]), \
        ...
        subjects_string_list = convert_entity_tokens_to_string_list(subjects, entity_names, entity_tokens)
        ...
        objects_string_list = convert_entity_tokens_to_string_list(objects, entity_names, entity_tokens)
...
def find_subject_for_verb(verb: spacy.tokens.Token, tokens_for_entities: list):
    """
    ...
    if verb.dep_ == Dependencies.ACL and is_entity_or_pronoun(verb.head, tokens_for_entities):
        return verb.head

    # 3) search recursively for a parent that has a desired subject dependency
    subject = find_subject_of_parent(verb.head, tokens_for_entities)
    if subject is not None:
        return subject
...
def find_subject_of_parent(parent: spacy.tokens.Token, tokens_for_entities: list):
    """
    ...
    def is_subject(token: spacy.tokens.Token, tokens_for_entities: list):
        ...
        return token.dep_ in SUBJECT_DEPS and is_entity_or_pronoun(token, tokens_for_entities)
    ...
    if parent == parent.head:
        ...
        return find_subject_of_parent(parent.head, tokens_for_entities)


def find_objects_for_verb(verb: spacy.tokens.Token, tokens_for_entities: list, wn_dictionary: WordNetDictionary):
    """
    ...
    for child in verb.children:
        if child.dep_ == Dependencies.DOBJ and is_entity_or_pronoun(child, tokens_for_entities):
            objects.append(child)
            modifiers_of_objects.append([])

    # 2) pobj: objects of preposition
    pobjs, modifiers_of_pobjs = find_pobj(verb, verb, tokens_for_entities, wn_dictionary)
    ...
    leaf_modifiers = find_leaf_modifiers(verb)
    leaf_modifiers = [t for t in leaf_modifiers if t.pos_ == Tags.ADP and is_preposition(t)]
    for modifiers_of_object in modifiers_of_objects:
        modifiers_of_object += leaf_modifiers
        sort_token_list(modifiers_of_object)
...
def find_pobj(token: spacy.tokens.Token,
    ...
    for child in token.children:
        if root_verb != token and child.dep_ == Dependencies.POBJ and is_entity_or_pronoun(child, tokens_for_entities):
        ...
        if (child.dep_, child.pos_) in VERB_MODIFIERS_FOR_POBJ:
            ...
            if child.pos_ == Tags.ADV and not wn_dictionary.is_wordnet_adverb(child):
                continue
            elif child.pos_ == Tags.ADP and not is_preposition(child):
                print(f"preposition {child} not known. Add it to PREPOSITIONS in entities_lib.py if desired.")
                continue
            pobjs_rec, modifiers_rec = find_pobj(child, root_verb, tokens_for_entities, wn_dictionary)
            pobjs += pobjs_rec
...
def find_leaf_modifiers(verb: spacy.tokens.Token):
    """
...
def convert_entity_tokens_to_string_list(tokens: list, entity_names: list, entity_tokens: list):
...
def find_conjunct_tokens_of_entities_or_prons(token: spacy.tokens.Token, tokens_for_entities: list):
    conjunct_tokens = [token]
    for child in token.children:
        if child.dep_ == Dependencies.CONJ and is_entity_or_pronoun(child, tokens_for_entities):
            conjunct_tokens += find_conjunct_tokens_of_entities_or_prons(child, tokens_for_entities)
    return conjunct_tokens


def get_sentence_idx_for_token(doc: spacy.tokens.Doc, token: spacy.tokens.Token):
    """
